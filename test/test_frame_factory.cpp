...
namespace
{
constexpr auto expected_frame_buffer =
    std::array<std::uint8_t, 9>{0xb5, 0x62, 0x20, 0x10, 0x01, 0x00, 0x20, 0x51, 0x03};
...
TEST_CASE("Frame factory shall put ubx preamble in frame buffer")
{
    ...
    frame_factory::create_frame(0x20, 0x00, payload_begin, payload_end, frame_buffer.begin(), frame_buffer.end());
...
TEST_CASE("Frame factory shall put message id in frame buffer")
{
    ...
    frame_factory::create_frame(0x20, 0x10, payload_begin, payload_end, frame_buffer.begin(), frame_buffer.end());
